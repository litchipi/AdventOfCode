...
impl Grid{
    ...
    fn validate_grid_inspect_range(&mut self, x: u32, y: u32, dx: i32, dy: i32) -> bool {
        !(((x == 0) & (dx < 0)) | ((y == 0) & (dy < 0)) | ((((x as i32)+dx) as u32) >= self.width) | ((((y as i32)+dy) as u32) >= self.height))
    }

    fn count_seats_around(&mut self, seat: GridEl) -> u32 {
        let mut count : u32 = 0;
        for direction in [(0, 1), (1, 0), (1, 1), (0, -1), (-1, 0), (-1, -1), (1, -1), (-1, 1)].iter() {
            let mut n : u32 = 0;
            count += loop {
                n += 1;
                if ! self.validate_grid_inspect_range(seat.x, seat.y, (n as i32)*direction.0, (n as i32)*direction.1){
                    break false;
                }
                let coord = (((seat.x as i32) + ((n as i32)*direction.0)) as u32,
                             ((seat.y as i32) + ((n as i32)*direction.1)) as u32);
                let seat = self.get_grid_el(coord.0, coord.1);
                if seat.is_floor{
                    continue
                }
                break seat.is_occupied;
            } as u32
        }
        count
    }
    ...
    fn check_occupied_seat(&mut self, i: usize) -> bool {
        if self.count_seats_around(self.seats[i]) >= 5 {
    ...
    fn check_empty_seat(&mut self, i: usize) -> bool {
        if self.count_seats_around(self.seats[i]) == 0{
