...
impl Tile {
    fn compare_border(&mut self, borders: [u16; 4], id: TileID) {
        for nb in 0..4 {
            //Nb border
            for nrot in 0..4 {
                let normal_found = if borders[nb] == self.get_border((nb + nrot) % 4) {
                    1 << ((nb + nrot) % 4)
                } else {
                    0
                };
                let reverse_found = if borders[nb]
                    == (self.get_border((nb + nrot) % 4).reverse_bits() >> (16 - 10))
                {
                    1 << ((nb + nrot) % 4)
                } else {
                    0
...
fn match_tiles(tiles: &mut Vec<Tile>) -> [TileID; 4] {
    ...
    let tiles_borders = tiles
        .iter()
        .filter(|x| x.count_borders() == 2)
        .collect::<Vec<&Tile>>();
    [
        tiles_borders[0].id,
        tiles_borders[1].id,
...
fn get_id_from_line(line: &str) -> TileID {
    line.split("Tile ").collect::<Vec<&str>>()[1]
        .split(":")
        .collect::<Vec<&str>>()[0]
        .parse::<TileID>()
        .unwrap() //.except("Cannot parse Tile ID")
}
